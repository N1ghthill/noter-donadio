...
export function renderPrometheusMetrics(snapshot: OperationalMetricsSnapshot): string {
  const lines = [
    ...
    '# HELP noter_media_downloads Number of media downloads by processing state.',
    '# TYPE noter_media_downloads gauge',
    ...PROCESSING_STATES.map((state) => `noter_media_downloads{state="${state}"} ${snapshot.mediaDownloads[state]}`),
